<?php
    ...
    if (count($orders) > 0) 
    {
        foreach($orders as $row)
        {
            ...
            {
                $is_received = "<span class='text-center badge-success'>Fully Received</span>";
                $to_receive = 1;
            }
            ...
            if($row['isPaid'] === 0 && $row['totalReceived'] > 0 && ($row['totalReceived'] < $row['totalQty']))
            {
                $buttons = '<div >
            ...
            if($row['isPaid'] === 0 && $row['totalReceived'] == 0)
            {
                $buttons = '<div >
                    <a id="btn_openUnpaidPayment" data-id='.$row['order_id'].' class="text-success productAnch " style="text-decoration: none;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#fff" class="bi bi-cash-stack" viewBox="0 0 16 16">
