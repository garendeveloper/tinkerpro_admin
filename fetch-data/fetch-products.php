...
if ($fetchProduct->rowCount() > 0) {
    while ($row = $fetchProduct->fetch(PDO::FETCH_ASSOC)) {
        ...
        <td class='text-center td-h discountTypes' style="width: 5%"  data-isSC = "<?= $row['isSC']?>" data-isPWD = "<?= $row['isPWD']?>" data-isNAAC = "<?= $row['isNAAC']?>" data-isSP = "<?= $row['isSP']?>" data-isMOV = "<?= $row['isMOV']?>"    ><?= $counter?><span hidden class="productsId"><?= $row['id'] ?></span><span hidden class="oumId"><?= $row['uom_id'] ?></span></td>
        ...
        <td class='status text-center td-h' style='padding: 2px; width: 6%; color: <?= ($row['status'] == 1) ? "green" : "red" ?>;'><?= ($row['status'] == 1) ? "Active" : "Inactive" ?></td>
        <td class='text-center action-td td-h' style="width: 6%; padding: 2px">
