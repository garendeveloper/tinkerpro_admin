...
if ($fetchProduct->rowCount() > 0) {
    while ($row = $fetchProduct->fetch(PDO::FETCH_ASSOC)) {
        ...
        <td class='status text-center td-h' style='width: 106px;color: <?= ($row['status'] == 1) ? "green" : "red" ?>;'><?= ($row['status'] == 1) ? "Active" : "Inactive" ?></td>
        <td class='text-center action-td td-h' style="width: 210px">
