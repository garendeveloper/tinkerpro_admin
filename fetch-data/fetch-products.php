...
if ($fetchProduct->rowCount() > 0) {
    while ($row = $fetchProduct->fetch(PDO::FETCH_ASSOC)) {
        ...
        <td class='text-center td-h' style="width: 37px;"><?= $counter?><span hidden class="productsId"><?= $row['id'] ?></span><span hidden class="oumId"><?= $row['uom_id'] ?></span></td>
        <td class='productsName text-left td-h' style='padding-left: 10px; width: 276px;'><?= $row['prod_desc']?></td>
        <td class='barcode text-center td-h' style="width: 141px;"><?= $row['barcode']?></td>
        <td class='sku text-center td-h' style="width: 112px;"><?= $row['sku']?></td>
        <td class='code text-center td-h' style="width: 113px;"><?= $row['code']?></td>
        <td class='uom_name text-center td-h' style="width: 112px;"><?= $row['uom_name'] ?? null ?></td>
        <td class='brand text-center td-h' style="width: 114px;"><?= $row['brand']?></td>
        <td class='prod_price text-end td-h pe-1' style="width: 112px;"><?= number_format((float)$row['prod_price'], 2, '.', '')?></td>
        <td class='markup text-end td-h pe-1' style="width: 114px;"><?= number_format((float)$row['markup'], 2, '.', '')?></td>
        <td class='cost text-end td-h pe-1' style="width: 113px;"><?= number_format((float)$row['cost'], 2, '.', '') ?></td>
        <td class='text-center td-h' style="width: 205px;">
        <?php
        ...
        <td class='status text-center td-h' style='padding: 2px; width: 113px; color: <?= ($row['status'] == 1) ? "green" : "red" ?>;'><?= ($row['status'] == 1) ? "Active" : "Inactive" ?></td>
        <td class='text-center action-td td-h' style="padding: 2px">
