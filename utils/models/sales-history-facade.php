...
class SalesHistoyFacade extends DBConnection {
    ...
    public function getAllSales($id, $roleId, $allUsers) {
        ...
        SELECT
            ...
            ROUND((((total) / 1.12) * 0.12)+ 0.0001, 2) AS VAT, 
            ROUND((ROUND(((total) / 1.12) + 0.0001, 2) * (customer_discount / 100))+ 0.0001, 2) AS fcustomer_discount,
            ((total * customer_discount) / 100) AS customerDisType,
            ROUND(total - (ROUND((ROUND(((total) / 1.12)+ 0.0001, 2) * (customer_discount / 100))+ 0.0001, 2)), 2) AS toBePaid,
            isVAT,
            is_paid,
            is_void,
            reason,
