...
{
    public function get_allExpensesDatatable($requestData)
    {
        $columns = [
            0 => 'expenses.id',
            1 => 'item_name',
        ...
        {
            $sql .= " WHERE expenses.item_name LIKE '%" . $requestData['search']['value'] . "%'
                    OR supplier.supplier LIKE '%" . $requestData['search']['value'] . "%'
                    OR uom.uom_name LIKE '%" . $requestData['search']['value'] . "%' 
                    OR expenses.billable_receipt_no LIKE '%" . $requestData['search']['value'] . "%' 
                    OR expenses.expense_type LIKE '%" . $requestData['search']['value'] . "%' 
                    OR expenses.invoice_number LIKE '%" . $requestData['search']['value'] . "%' ";
        }
    ...
    public function get_allExpenses()
    {
        ...
        $data = $this->get_allExpensesDatatable($requestData);
    ...
    public function save_expense($formdata)
    {
        ...
        $fields_to_validate = ['item_name', 'date_of_transaction',  'qty', 'expense_type'];
        $field_labels = [
            'item_name' => 'Item Name',
