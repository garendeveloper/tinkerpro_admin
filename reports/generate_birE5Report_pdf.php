...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->Cell(0, 1, 'ANNEX "E-5"', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        $this->Ln();
...
$pdf->SetSubject('E-5 Solo Parent Sales Book/Report');
$pdf->SetKeywords('TCPDF, PDF, Sales, Solo Parent, guide');
...
$pdf->SetFont('', 'B', 10);
$maxCellHeight = 8; 
$pdf->Cell(277, 8, "Solo Parent Sales Book/Report", 1, 0, 'C', true);
$pdf->Ln();
$header = array('Date','Name of Solo Parent','SPIC No.', 'Name of child', 'Birth Date of child', 'Age of child', 'SI / OR Number', 'Gross Sales', 'Discount', 'Net Sales');
$headerWidths = array(20, 40, 25, 37, 25, 25, 25, 25, 25, 30);
$maxCellHeight = 13; 
...
foreach ($header as $key => $heading) 
{
    ...
    if($key === 0) $hexColor = '#A6A6A6';//gray
    if($key === 1) $hexColor = '#ED7D31';//darkorange
    if($key === 2) $hexColor = '#FFC000';//lightorange
    if($key === 3 || $key === 4 || $key === 5) $hexColor = '#FFFF00';//yellow
    if($key === 6) $hexColor = '#92D050';//lightgreen
    if($key === 7) $hexColor = '#00B0F0';//lightblue
    if($key === 8) $hexColor = '#9999FF';//lightviolet
    if($key === 9) $hexColor = '#FF66FF'; //magenta
    list($r, $g, $b) = sscanf($hexColor, "#%02x%02x%02x");
    $pdf->SetFillColor($r, $g, $b);
...
if(count($items) > 0)
{
    foreach ($items as $key =>$item) 
    {
        ...
        $pdf->SetFont('', '', autoAdjustFontSize($pdf, $child_name, $headerWidths[3]));
        $pdf->Cell($headerWidths[3], $maxCellHeight, $child_name, 1, 0, 'C');
        $pdf->SetFont('', '', autoAdjustFontSize($pdf, $child_birth, $headerWidths[4]));
        $pdf->Cell($headerWidths[4], $maxCellHeight, $child_birth, 1, 0, 'C');
        $pdf->SetFont('', '', autoAdjustFontSize($pdf, $child_age, $headerWidths[5]));
        $pdf->Cell($headerWidths[5], $maxCellHeight, $child_age, 1, 0, 'C');
        $pdf->SetFont('', '', autoAdjustFontSize($pdf, $item['barcode'], $headerWidths[6]));
        $pdf->Cell($headerWidths[6], $maxCellHeight, $item['barcode'], 1, 0, 'C');
        $pdf->SetFont('', '', autoAdjustFontSize($pdf, number_format($item['totalAmount'], 2), $headerWidths[7]));
        $pdf->Cell($headerWidths[7], $maxCellHeight, number_format($item['totalAmount'], 2), 1, 0, 'R');
        $pdf->SetFont('', '', autoAdjustFontSize($pdf, number_format($item['customer_discount'], 2), $headerWidths[8]));
        $pdf->Cell($headerWidths[8], $maxCellHeight, number_format($item['customer_discount'], 2), 1, 0, 'R');
