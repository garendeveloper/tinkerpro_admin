<?php
    ...
    $pdf->SetTitle('Loss and Damage Table PDF');
    $pdf->SetSubject('Loss and Damage PDF Document');
    $pdf->SetKeywords('TCPDF, PDF, Loss and Damage, table');
    ...
    $pdf->SetFont('', 'B', 10);
    $pdf->Cell(0, 10, 'Loss and Damage : '.$_GET['ld_reason'], 0, 1, 'R', 0); 
    $pdf->Ln(-5);
    $pdf->SetFont('',  10);
    ...
    $pdf->Ln(-6);
    $current_date = date('F j, Y');
    $pdf->Cell(0, 10, "Date Damage: {$_GET['date_damage']}", 0, 'L');
    $pdf->Ln(-6);
    $current_date = date('F j, Y');
    ...
    ob_end_clean(); 
    $pdf->Output('lossanddamage.pdf', 'I');
    $pdfPath = __DIR__ . '/assets/pdf/inventory/lossanddamage.pdf';
